...
namespace server {
    ...
    void setupdemoplayback()
    {
        ...
        if(!demoplayback) formatstring(msg)("\f3Error: \f7couldn't read demo \"%s\"", file);
					...
					bool revoke = false)
    {
        ...
        if(val)
        {
            ...
            else if(wantpriv <= PRIV_MASTER && !force)
            {
                ...
                {
                    sendf(ci->clientnum, 1, "ris", N_SERVMSG, "\f3Error: Master is already claimed.");
                    return false;
                }
                if(!authname && !(mastermask&MM_AUTOAPPROVE) && !ci->privilege && !ci->local)
                {
                    sendf(ci->clientnum, 1, "ris", N_SERVMSG, "\f3Error: Server mode requires \"/auth\" type authentication.");
                    return false;
                }
            }
   ...
   // extern void loaditems();
    ...
    void vote(const char *map, int reqmode, int sender)
    {
        ...
        {
            sendf(sender, 1, "ris", N_SERVMSG, "\f3Error: Custom map selection disabled (Map rotation locked).");
            return;
        }
...
best.add(clients[i]); \
    ...
    {
        //if(ci->state.state==CS_ALIVE) game::suicide(ci);
    ...
    void dodamage(clientinfo *target, clientinfo *actor, int damage, int gun, const vec &hitpush = vec(0, 0, 0))
    {
        ...
        if(ts.health<=0)
        {
            ...
            loopv(spreemessages) {
                if(actor->state.spreefrags == spreemessages[i].frags) out(ECHO_SERV, "\f0%s \f4%s \f6%s", colorname(actor), spreemessages[i].msg1, spreemessages[i].msg2);
            }
        ...
        while((curmsg = p.length()) < p.maxlen) switch(type = checktype(getint(p), ci)) {
            ...
            case N_TEXT: {
                ...
                if(text[0] == '#') {
                    char *c = text;
                    while(*c && isspace(*c)) c++;
                    if(!qs.handleTextCommands(ci, text)) {
                        break;
                    }
                }
                else {
                    ...
                    else {sendf(ci->clientnum, 1, "ris", N_SERVMSG, "\f3You are muted.");}
                }
            ...
            case N_EDITVAR:
            {
                ...
                QUEUE_MSG; //if(ci && ci->state.state!=CS_SPECTATOR)
                break;
            }
            ...
            case N_MASTERMODE:
            {
                ...
                if((ci->privilege || ci->local) && mm>=MM_OPEN && mm<=MM_PRIVATE)
                {
                    ...
                    {
                        defformatstring(s)("\f3Error: Mastermode %d is disabled on this server", mm);
            ...
            case N_GETMAP:
                ...
                else if(ci->getmap) sendf(sender, 1, "ris", N_SERVMSG, "\f3Error: already sending map");
                else
                {
